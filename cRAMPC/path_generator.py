...
class PathGenerator(Node):
    ...
    def __init__(self):
        """Construct the Path Generator Node."""
        ...
        self.declare_parameter('ref_point', [0.0, 0.0, 0.0])
        ...
        if self.ref_type == 'ref':
            self.path_pub = self.create_publisher(Pose2D, 'trajectory', 10)
            self.ref_point = (
                self.get_parameter('ref_point').get_parameter_value().double_array_value
            )
        ...
        self.cmd_sub = self.create_subscription(
            Bool, 'cmd_done', self.cmd_callback, 10
        )
        ...
        initial_position = self.get_parameter('initial_position').get_parameter_value().double_array_value
    ...
    def odom_callback(self, msg):
        """Receive odometry data to update the current state."""
        ...
        if self.odom_type == 'full':
            ...
            self.current_position = [msg.twist.twist.linear.x,
                                     msg.twist.twist.linear.y, msg.twist.twist.angular.z]

        elif self.odom_type == 'velocity_orientation':
            self.current_position = [delta_theta, msg.twist.twist.linear.x]
    ...
    def publish_path(self):
        ...
        #     if (distance_to_goal < 0.1 and self.last_index + self.horizon+1 < len(self.path.array) - 1):
        #         self.last_index += 1
        #         path_to_publish.array = self.path.array[self.last_index: self.last_index + self.horizon+1]

        #     elif distance_to_goal < 0.1 and self.last_index < len(self.path.array) - 1:
        #         path_to_publish.array = self.path.array[self.last_index:] + self.path.array[:(len(self.path.array)-self.last_index)]
        ...
        #             self.last_index: self.last_index + self.horizon+1
        #         ]
        ...
        #     circle_pose = np.array([[0.5*np.cos(a)-0.5, 0.5*np.sin(a), a] for a in circle_angle])
        #     self.path.data = list(circle_pose[self.last_index+1])
        #     self.last_index = (self.last_index + 1)
        #     print(list(circle_pose[self.last_index]))
    ...
    def load_trajectory_from_file(self):
        ...
        if self.file_path == 'None':
            ...
            with open(self.file_path, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    self.path[0].append(float(row['x']) + self.current_position[0])
                    self.path[1].append(float(row['y']) + self.current_position[1])
                    self.path[2].append(float(row['theta']) + self.current_position[2])
